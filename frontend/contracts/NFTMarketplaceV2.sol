...
contract NFTMarketplaceV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721URIStorageUpgradeable 
{
    ...
    function initializeV2(address payable _commissionReceiver, uint256 _commissionPercentage) public reinitializer(2) 
    {
        commissionReceiver = _commissionReceiver;
        commissionPercentage = _commissionPercentage;
    }
    
    function setCommissionReceiver(address payable _newReceiver) external onlyOwner 
    {
    ...
    {
        require(_newPercentage <= 100, "Commission percentage cannot be more than 100");
    ...
    function purchaseMarketItem(uint256 tokenId) public payable
    {
        ...
        payable(idToMarketItem[tokenId].seller).transfer(remainingAmount);
        idToMarketItem[tokenId].owner = payable(msg.sender);
